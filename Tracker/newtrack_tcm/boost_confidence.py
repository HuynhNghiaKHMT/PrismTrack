...
def dets_to_xyxy(dets):
    ...
    for d in dets:
        ...
        arr.append([x1, y1, x2, y2, d.det_score])
...
def update_det_scores(dets, new_scores):
    for i in range(len(dets)):
        dets[i].det_score = np.float32(new_scores[i])
        # dets[i].new_score = np.float32(new_scores[i])
    return dets
...
def clamp_dlo_mean_global(dets, tracks, delta=0.2): # 68.922

    if len(tracks) == 0 or len(dets) == 0:
        return dets

    kf_scores = np.array([t.kf_score for t in tracks])
    mean_kf = np.mean(kf_scores)
...
def clamp_dlo_per_track(dets, tracks, delta=0.5): # 68.799

    if len(tracks) == 0 or len(dets) == 0:
        return dets

    # Compute IoU matrix
    iou_sim, _ = diou_distance(tracks, dets)  # shape: [T, D]

    for j, d in enumerate(dets):

        # tìm track có IoU lớn nhất
        iou_col = iou_sim[:, j]
        best_track_idx = np.argmax(iou_col)
        best_iou = iou_col[best_track_idx]

        # nếu không đủ gần → bỏ qua clamp
        if best_iou < 0.1:
            continue

        ref_score = tracks[best_track_idx].kf_score

        # clamp quanh track đó
        d.new_score = np.clip(
            d.new_score,
            ref_score - delta,
            ref_score + delta
...
def dlo_confidence_boost_v2(
    ...
    # =========================
    if not use_sb and not use_vt:
        scores[low_mask] = np.maximum(
            scores[low_mask],
            max_s[low_mask] * dlo_boost_coef
        )
        ...
        if use_sb:
            ...
            scores[low_mask] = np.maximum(
                scores[low_mask],
                alpha * scores[low_mask] +
                (1 - alpha) * (max_s[low_mask] ** 1.5)
            )
        ...
        if use_vt:
            ...
            idx = np.where(low_mask & tmp)[0]
            scores[idx] = np.maximum(scores[idx], det_thresh + 1e-5)

    # =========================
    # 🔥 HIGH DET → SMOOTH NHẸ (NEW)
    # =========================
...
def duo_confidence_boost_v2(dets, tracks, frame_id, iou_limit, det_thresh):
    detections = dets_to_xyxy(dets)
    limit = 13.2767
    ...
    if dist.size > 0:
        ...
        # 🔥 CHỈ BOOST det_low
        low_mask = detections[:, 4] < det_thresh
        mask = (min_dist > limit) & low_mask
