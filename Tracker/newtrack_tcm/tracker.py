from newtrack_tcm.cmc import *
from newtrack_tcm.utils import *
from newtrack_tcm.track_score import *
from newtrack_tcm.association import *
from newtrack_tcm.boost_confidence import *
...
class Tracker(object):
    ...
    def init_tracks(self, dets):
        ...
        if use_tai:
            ...
            iou_sim, iou_dist = diou_distance(tracks + dets, tracks + dets)

            scores = np.array([d.new_score for d in dets])
    ...
    def update(self, dets, use_cmc, use_dlo, use_reid, use_tpa):
        ...
        # 3. CLAMP DLO (🔥 QUAN TRỌNG)
            # dets = clamp_dlo_mean_global(dets, self.tracks, delta=0.2) # 0.25: 69.219 45, 0.2: 68.840 41
            # dets = clamp_dlo_per_track(dets, self.tracks, delta=0.5)

        # Split detections
        dets_high = [d for d in dets if d.det_score > self.args.det_high_thr]
        dets_low  = [d for d in dets if self.args.det_low_thr <= d.det_score <= self.args.det_high_thr]
        # dets_high = [d for d in dets if d.new_score >= self.args.det_high_thr]
        # dets_low  = [d for d in dets if self.args.det_low_thr <= d.new_score < self.args.det_high_thr]
        ...
        if use_cmc:
            ...
            apply_cmc_safe(tracked_lost, warp_matrix)
            apply_cmc_safe(new_tracks, warp_matrix)
        ...
        u_track = []
        dets_high_remain = []
        use_mutil_stage = True # True: tpa stage 1 + stage 2, False: tpa stage 1-2 merged
        ...
        if not use_tpa:
            ...
            if not use_mutil_stage:
                ...
                matches, u_tracks, u_dets = tpa_assignment(tracked_lost, dets_all, cost, self.args.match_thr, self.args.reduce_step)
                ...
                cost = build_cost_stage1(tracked_lost, dets_high, self.frame_id, use_reid)
                matches1, u_tracks1, u_dets1 = tpa_assignment(tracked_lost, dets_high, cost, self.args.match_thr, self.args.reduce_step)

                for t, d in matches1:
                    tracked_lost[t].update(self.frame_id, dets_high[d], update_feat=True)

                remain_tracked = [tracked_lost[i] for i in u_tracks1]

                cost = build_cost_stage2(remain_tracked, dets_low, self.frame_id)
                matches2, u_tracks2, u_dets2 = tpa_assignment(remain_tracked, dets_low, cost, 0.5, self.args.reduce_step)
                ...
                # unmatched tracked -> lost
                for i in u_tracks2:
                    if remain_tracked[i].state == TrackState.Tracked:
                        remain_tracked[i].mark_lost()
        ...
        # matches3, u_tracks3, u_dets3  = tpa_assignment(stage3_tracks, dets_high_remain, cost, self.args.match_thr, self.args.reduce_step)
    ...
    def update_without_detections(self):
        ...
        if self.cmc is not None: # change
            ...
            apply_cmc_safe(self.tracks, warp_matrix)
