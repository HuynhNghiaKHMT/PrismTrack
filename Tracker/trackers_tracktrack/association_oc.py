...
def build_cost(tracks, dets, args, frame_id, use_reid):
    ...
    # fuse motion
    iou_dist = 0.7 * iou_dist + 0.3 * maha_cost
    ...
    if use_reid:
        ...
        """
        cost = 0.4 * iou_dist + 0.6 * cos_dist
    ...
    '''
    angle_cost = angle_distance(tracks, dets, frame_id)

    for t, track in enumerate(tracks):

        v = np.linalg.norm(track.velocity.mean(axis=0))

        weight = min(0.3, 0.05 + 0.1 * v)

        cost[t] += weight * angle_cost[t]
    '''
