...
class CMC:
    def __init__(self, vid_name):
        ...
        self.gmcFile = open('Tracker/trackers/cmc/' + 'GMC-' + vid_name + ".txt", 'r')
...
def apply_cmc_safe(tracks, warp_matrix):
    if len(tracks) == 0:
        return

    # Backup score-related dims
    extra_means = []
    extra_covs = []

    for t in tracks:
        if t.mean.shape[0] == 10:
            extra_means.append(t.mean[8:].copy())
            extra_covs.append(t.covariance[8:, 8:].copy())

            # shrink to 8D
            t.mean = t.mean[:8]
            t.covariance = t.covariance[:8, :8]
        else:
            extra_means.append(None)
            extra_covs.append(None)

    # Apply original CMC
    apply_cmc(tracks, warp_matrix)

    # Restore
    for i, t in enumerate(tracks):
        if extra_means[i] is not None:
            mean_8 = t.mean
            cov_8 = t.covariance

            new_mean = np.zeros(10)
            new_mean[:8] = mean_8
            new_mean[8:] = extra_means[i]

            new_cov = np.zeros((10, 10))
            new_cov[:8, :8] = cov_8
            new_cov[8:, 8:] = extra_covs[i]
