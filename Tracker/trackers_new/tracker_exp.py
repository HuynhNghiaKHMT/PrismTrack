...
from trackers_new.association import *
from trackers_new.boost_confidence import apply_boost
...
class Tracker(object):
    ...
    def init_tracks(self, dets):
        ...
        if use_tai:
            ...
            # 🔥 DEBUG 
            # print(f"[TAI] before={len(dets)} after={np.sum(allow_indices)}")
    ...
    def update(self, dets, use_cmc, use_dlo, use_reid, use_tpa):
        ...
        for t, d in matches3:
            stage3_tracks[t].update(self.frame_id, dets_high_remain[d], update_feat=False)
