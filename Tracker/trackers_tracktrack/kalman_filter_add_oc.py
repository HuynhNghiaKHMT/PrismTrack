...
class HybridKalmanFilter:
    ...
    def __init__(self):
        ...
        # add for oc sort
        self.last_observation = None # Lưu z cuối cùng
        self.last_observed_frame = 0        # Lưu lịch sử để smoothing
        self.observed = False
    ...
    def online_smoothing(self, mean, covariance, current_z, current_frame):
        """
        Sử dụng frame_id để tính time_gap chính xác
        """
        last_z = self.last_observation
        # Tính khoảng cách frame thực tế
        time_gap = current_frame - self.last_observed_frame
        if time_gap <= 0: time_gap = 1
        
        # Tính vận tốc ảo: (vị trí mới - vị trí cũ) / khoảng thời gian
        virtual_velocity = (current_z - last_z) / time_gap
        
        # Cập nhật vận tốc vào state (vx, vy, vw, vh nằm ở index 4,5,6,7)
        mean[4:] = virtual_velocity
        
        # Giảm hiệp phương sai (giảm độ bất định)
        covariance *= 0.8 
        
        return mean, covariance

    def update(self, mean, covariance, z, frame_id, confidence = .0):

        # Kiểm tra nếu track vừa quay trở lại sau khi mất dấu
        if not self.observed and self.last_observation is not None:
            mean, covariance = self.online_smoothing(mean, covariance, z, frame_id)
