...
class GMC:
    def __init__(self, method='sparseOptFlow', downscale=2, verbose=None):
        ...
        if self.method == 'orb':
            ...
            root_folder = fr'Tracker/CMC/cmc/'
            if not os.path.exists(root_folder):
                os.makedirs(root_folder)

            if "MOT" in seqName:
                seqName = seqName.split("-FRCNN")[0]
            elif "dance" in seqName:
                seqName = 'dancetrack-' + seqName.split('dancetrack')[1]

            gmc_path = fr'{root_folder}/GMC-{seqName}.txt'
            if not os.path.exists(gmc_path):
                with open(gmc_path, "w") as f:
                    pass

            self.gmc_file = open(gmc_path, 'w')
            self.count = 0
    ...
    def applySparseOptFlow(self, raw_frame, detections=None):
        ...
        if not self.initializedFirstFrame:
            ...
            # Initialization done
            self.initializedFirstFrame = True
            gmc_line = str(self.count) + "\t" + str(H[0, 0]) + "\t" + str(H[0, 1]) + "\t" + str(
...
def make_parser():
    parser = argparse.ArgumentParser("GMC argument")
...
def run_cmc(args):
    ...
    for vid in sorted(os.listdir(data_dir)):
        ...
        if "SDP" in vid or "DPM" in vid:
            continue
            
        ## Initialize gmc
        gmc = GMC(method=args.method, verbose=[vid, args.mode])

        ## Loop through image
        img_dir = os.path.join(data_dir, vid, "img1")
        for image in tqdm.tqdm(sorted(os.listdir(img_dir)), desc=f"Video name: {vid}"):
