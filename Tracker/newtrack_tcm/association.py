...
def build_cost_stage1(tracks, dets, frame_id, use_reid):
    ...
    if use_reid:
        ...
        # cost += 0.15 * conf_distance_kf(tracks, dets) + 0.05 * angle_distance(tracks, dets, frame_id, 3) # HMIoU
        # cost += 0.15 * conf_distance_kf(tracks, dets)  + 0.05 * angle_distance(tracks, dets, frame_id, 3) # DIoU
        # cost += 0.15 * conf_distance_kf(tracks, dets) + 0.05 * angle_distance(tracks, dets, frame_id, 3) # DLO
...
def build_cost_stage2(tracks, dets, frame_id):
    ...
    # cost += 0.15 * conf_distance_linear(tracks, dets) + 0.05 * angle_distance(tracks, dets, frame_id, 3) # HMIoU
    # cost += 0.15 * conf_distance_linear(tracks, dets) # DIoU
    # cost += 0.15 * conf_distance_linear(tracks, dets) + 0.05 * angle_distance(tracks, dets, frame_id, 3) # DLO
    cost += 0.15 * conf_distance_linear(tracks, dets) + 0.05 * angle_distance(tracks, dets, frame_id, 3) # KF score
...
def tpa_assignment(tracks, dets, cost, match_thr, reduce_step = 0.05):
    ...
    u_dets   = [i for i in range(len(dets)) if i not in matched_d]
...
def build_cost_stage12_v2(tracks, dets, frame_id, use_reid):
