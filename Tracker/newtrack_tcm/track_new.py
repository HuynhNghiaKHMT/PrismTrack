...
from newtrack_tcm.kalman_filter_hybird_new import HybridKalmanFilterNew
...
class Track(BaseTrack):
    ...
    def initiate(self, frame_id, counter):
        ...
        self.kalman_filter = HybridKalmanFilterNew()
            
        self.mean, self.covariance = self.kalman_filter.initiate(self.cxcywh.copy(),  self.det_score.copy())
        self.kf_score = self.mean[4]
    ...
    def update(self, frame_id, detection, update_feat=True):
        ...
        # alpha =  0.35
        # fused_score = alpha * self.mean[4] + (1 - alpha) * detection.new_score
